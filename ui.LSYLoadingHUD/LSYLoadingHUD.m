...
#define StateFont [UIFont systemFontOfSize:21]
#define FontColor [UIColor colorWithRed:213/255.f green:213/255.f blue:213/255.f alpha:1.0]
...
- (void)viewDidLoad {
    ...
    if (self.backgroundColor) {
        [self.view setBackgroundColor:[self.backgroundColor copy]];
    }
}
...
- (void)viewDidLayoutSubviews {
    ...
    if (self.topSpacesRatio > 0) {
        _activity.frame = CGRectMake((self.view.frame.size.width - 100)/2, self.view.frame.size.height*self.topSpacesRatio-100, 100, 100);
    }
    ...
    _stateLabel.frame = CGRectMake(10,
                                   ((_activity.frame.origin.y+_activity.frame.size.height)?:(_loadingView.frame.origin.y+_loadingView.frame.size.height)),
                                   self.view.frame.size.width,
                                   [_stateLabel sizeThatFits:CGSizeMake(self.view.frame.size.width, 0)].height
                                   );
    _refreshButton.frame = CGRectMake(
                                      (self.view.frame.size.width-100)/2,
                                      _stateLabel.frame.origin.y+_stateLabel.frame.size.height+10,
                                      100,
                                      30);
}
...
- (void)hiddenHUDViewController:(UIViewController *)vc {
     [self p_loadStateShowWithState:OKWLoadEnd];
...
+ (void)hiddenAllHUD:(UIViewController *)vc {
    for (UIViewController *viewController in vc.childViewControllers) {
        if ([viewController isKindOfClass:[LSYLoadingHUD class]]) {
            ...
            [loadingVC hiddenHUDViewController:vc];
        }
    }
}
