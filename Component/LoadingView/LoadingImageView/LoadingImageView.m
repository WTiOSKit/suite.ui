...
- (void)commonInitWithSize:(CGSize)size {
    self.backgroundColor = color_with_rgb(204, 204, 204);
    ...
    retryTipLabel.textColor = color_with_rgb(255, 255, 255);
    ...
    [retryButton setBackgroundColor:color_with_rgb(240, 240, 240)];
    [retryButton setTitleColor:color_with_rgb(100, 100, 100) forState:UIControlStateNormal];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    if (self.shouldStrictTap) {
        self.isMove = YES;
    }
    
    // 非严格单击下，不设置该标记。。。@fallenink：有历史原因
}
