...
//  hairdresser
//
//  Created by fallen.ink on 8/9/16.
//
//
...
- (void)setTextHeightChangeHandler:(void (^)(NSString *, CGFloat))textChangeHandler {
    _textHeightChangeHandler = textChangeHandler;
...
- (void)textDidChange {
    ...
    if (_textH != height) { // 高度不一样，就改变了高度
        ...
        if (self.textHeightChangeHandler && self.scrollEnabled == NO) {
            self.textHeightChangeHandler(self.text,height);
