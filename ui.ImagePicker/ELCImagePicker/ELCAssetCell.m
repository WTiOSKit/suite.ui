...
- (void)setAssets:(NSArray *)assets
{
    ...
    for (int i = 0; i < [_rowAssets count]; ++i) {
        ...
        __unused ALAssetRepresentation *representation = asset.asset.defaultRepresentation;
        CGImageRef image = asset.asset.aspectRatioThumbnail; // asset.asset.thumbnail; //representation.fullScreenImage
        ...
        if (i < [_overlayViewArray count]) {
            ...
            overlayView.labIndex.hidden = !elcAppearance.showOrderNum;
        } else {
            if (overlayImage == nil) {
                overlayImage = image_named(elcAppearance.selectedImageName);
            }
...
+ (CGFloat)imageItemWidthAndHeight
{
    ...
    if (kImageItemWidthAndHeight <= 1e-6) {
        kImageItemWidthAndHeight= (screen_width - kImageItemPadding * (kRowImagesCount + 1)) / kRowImagesCount;
    }
