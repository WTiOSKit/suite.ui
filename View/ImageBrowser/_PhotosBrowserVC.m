...
- (void)showWithImageView:(QQingImageView *)imageView withCompletion:(Block)completeBlock {
    ...
    self.view.tag = kTagOfQQingPhotosBrowserVCView;
    [window addSubview:self.view];
    [[QQingUtils topVisibleChildViewController] addChildViewController:self];
...
- (void)setupPageControl {
    if (!_pageControl) {
        ...
        _pageControl.highlightedImage = [UIImage circleImageWithColor:[g_commonConfig themeColor] andSize:CGSizeMake(8 * screenScale, 8 * screenScale)];
...
- (IBAction)savePhotoToLocalAction:(id)sender {
    if (![QQingUtils isAlbumAccessible]) {
        [UIUtils showAlertView:@"" :QQingString(@"公共部分.申请手机相册访问权限.弹框内容") :@"知道了"];
    } else {
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    if(error != NULL) {
        [QQingUtils showToastWithText:QQingString(@"图片浏览器.图片保存失败.Toast提示")];
    } else {
...
- (IBAction)deleteAction:(id)sender {
    QQingAlertView *alertView = [[QQingAlertView alloc]
                                 initWithTitle:nil
                                 message:QQingString(@"图片浏览器.确认删除图片.弹框内容")
                                 delegate:self
