...
- (void)viewWillAppear:(BOOL)animated{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        CGFloat centerX = (_selectedTabIndex + 0.5 ) * (kScreenWidth / self.btnArray.count);
...
- (void)setupTitleView
{
    ...
    for (NSInteger i = 0; i < count; i++) {
        ...
        reddotV.backgroundColor = [UIColor themeRedColor];
        ...
        newTagV.backgroundColor = [UIColor themeRedColor];
...
- (UIView *)indicatorView{
    if (_indicatorView == nil) {
        ...
        CGFloat viewH = 2;
        CGFloat viewW = (_indicatorWidth ? _indicatorWidth : (kScreenWidth/ self.childViewControllers.count) * .7);
        CGFloat viewY = (kTitleViewHeight - viewH);
        CGFloat viewCenterX = (kScreenWidth/ self.childViewControllers.count ) *.5;
...
- (void)addMyChildView{
    ...
    [contentView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.mas_equalTo(kScreenWidth* self.childViewControllers.count);
    }];
...
- (void)clickTitleBtn:(UIButton *)sender {
    ...
    if (self.animationType == QQTitleDisplayAnimationTypeDelayed) {
        CGFloat centerX = (index + 0.5 ) * (kScreenWidth / self.btnArray.count);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    if (self.animationType == QQTitleDisplayAnimationTypeDefault) {
        CGFloat centerX = kScreenWidth/(self.childViewControllers.count*2) + scrollView.contentOffset.x/self.childViewControllers.count;
        [self.indicatorView mas_updateConstraints:^(MASConstraintMaker *make) {
            make.centerX.equalTo(self.titleView.mas_left).offset(centerX);
        }];
    }
...
- (UIColor *)normalTitleColor{
    if (_normalTitleColor == nil) {
        _normalTitleColor = [UIColor gray006Color];
    }
...
- (UIColor *)selectTitleColor{
    if (_selectTitleColor == nil) {
        _selectTitleColor = [UIColor themeColor];
    }
...
- (UIColor *)titleSplitColor{
    if (_titleSplitColor == nil) {
        _titleSplitColor = [UIColor gray001Color];
    }
